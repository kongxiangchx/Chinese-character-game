...
def getHeaders():
    user_agents =  ['Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36','Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.102 Safari/537.36 Edge/18.18362']
    headers = {'User-Agent':random.choice(user_agents)}
    return headers
def getHtml(url):
    try:
        time.sleep(random.random())
        req=requests.get(url,headers=getHeaders(),proxies=proxies,timeout=5)
...
def getProxies():
    ...
    for i in range(0,8):
        proxies[result[i][10]].append(result[i][10]+'://'+result[i][0]+':'+result[i][2])
