    ...
    def click1(self):
        ...
        if acc != ''.join(acc.split()) or pass1 != ''.join(pass1.split()) or nickname != ''.join(nickname.split()):
            QMessageBox.information(
                self, "注册", "个人信息不能包含空白字符", QMessageBox.Yes)
            return
        if pass1 != pass2:
            QMessageBox.information(self, "注册", "两次输入的密码必须相同", QMessageBox.Yes)
            return
        data = {'type': 'register', 'acc': acc,
                'passwd': pass1, 'nickname': nickname, 'sex': sex}
        s = Send_data()
        recv = s.message(data)
        s.close()
...
class PoemGame(QMainWindow, Ui_poem_game):
    ...
    def send_result(self):
        data = {'type': 'poem_result', 'acc': user,
                'poem_score': self.ans_num.intValue()}
        s = Send_data()
        recv = s.message(data)
        s.close()
        QMessageBox.information(self, "答题结束", "共作答"+str(self.ans_num.intValue()
                                                        )+'道题，打败全国'+recv['rate']+'的用户', QMessageBox.Yes)
        self.hide()
    ...
    def click1(self):
        ...
        if flag == 0:
            ...
            QMessageBox.information(
                self, "答题结果", "答案错误，正确答案是"+self.ans, QMessageBox.Yes)
    ...
    def click3(self):
        reply = QMessageBox.question(
            self, '退出答题', '你确定要退出吗?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
class IdiomGame(QMainWindow, Ui_idiom_game):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.bt_list = [self.bt1, self.bt2, self.bt3, self.bt4, self.bt5, self.bt6, self.bt7, self.bt8, self.bt9, self.bt10, self.bt11, self.bt12, self.bt13, self.bt14,
                        self.bt15, self.bt16, self.bt17, self.bt18, self.bt19, self.bt20, self.bt21, self.bt22, self.bt23, self.bt24, self.bt25, self.bt26, self.bt27, self.bt28]
        for i in range(28):
            self.listen(self.bt_list[i], i)
        self.fi_lb = [self.fi_lb1, self.fi_lb2, self.fi_lb3,
                      self.fi_lb4, self.fi_lb5, self.fi_lb6, self.fi_lb7]
        self.tip_bt.clicked.connect(self.click2)
        self.exit_bt.clicked.connect(self.click3)
    ...
    def send_result(self):
        data = {'type': 'idiom_result', 'acc': user,
                'idiom_score': self.time_cnt}
        s = Send_data()
        recv = s.message(data)
        s.close()
        QMessageBox.information(
            self, "答题结束", "共作答"+str(self.time_cnt)+'s，打败全国'+recv['rate']+'的用户', QMessageBox.Yes)
        self.hide()
    ...
    def click3(self):
        self.timer.stop()
        reply = QMessageBox.question(
            self, '退出游戏', '你确定要退出吗?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.hide()
        else:
...
class PerInfo(QMainWindow, Ui_per_info):
    ...
    def click2(self):
        ...
        if pass1 == '' or nickname == '':
            QMessageBox.information(
                self, "修改个人信息", "个人信息不能为空", QMessageBox.Yes)
            return
        if pass1 != ''.join(pass1.split()) or nickname != ''.join(nickname.split()):
            QMessageBox.information(
                self, "修改个人信息", "个人信息不能包含空白字符", QMessageBox.Yes)
            return
        if pass1 != pass2:
            QMessageBox.information(
                self, "修改个人信息", "两次输入的密码必须相同", QMessageBox.Yes)
            return
        data = {'type': 'update_info', 'acc': acc,
                'passwd': pass1, 'nickname': nickname, 'sex': sex}
        s = Send_data()
        recv = s.message(data)
        s.close()
